...
public class Student implements IStudent {
    ...
    @Override
    public void inputData(Scanner scanner, Student[] students, int numStudent) {
        System.out.print("Nhập mã sinh viên: ");
        boolean checkStudentId;
        do {
            this.studentId = scanner.nextLine();
            checkStudentId = uniqueStudentId(students, studentId, numStudent);
            if (checkStudentId) {
    ...
    public boolean uniqueStudentId(Student[] students, String studentId, int numStudent) {
        for (int i = 0; i < numStudent; i++) {
            if (students[i].studentId.equals(this.studentId)) {
                return true;
            }
        }
        return false;
    ...
    public void StudentsByRank(Student[] students, int numStudent) {
        if (numStudent == 0) {
            System.out.println("Không có sinh viên nào trong danh sách.");
        }
        int[] studentsByRank = new int[numStudent];
        String[] arrRank = new String[numStudent];

        int studentCount = 0;
        for (int i = 0; i < numStudent; i++) {
            String student = students[i].getRank();
    ...
    public void studentsByStatus(Student[] students, int numStudent) {
        if (numStudent == 0) {
            System.out.println("Không có sinh viên nào trong danh sách.");
            return;
        }
        int[] studentsByStatus = new int[numStudent];
        String[] arrStatus = new String[numStudent];
        int studentCount = 0;

        for (int i = 0; i < numStudent; i++) {
            String student = students[i].getStatus();
            boolean isExist = false;
            for (int j = 0; j < studentCount; j++) {
